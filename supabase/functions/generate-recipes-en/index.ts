...
function buildPrompt(params: {
  ...
  const temperatureRule = params.canAdjustTemperature
    ? "The user can adjust temperature."
    : "The user CANNOT adjust temperature. Set temperature to 100°C.";

  const comment = params.userComment
    ? `Additional user comment: ${params.userComment}`
    : "No additional comment.";

  return [
    "Generate EXACTLY 3 coffee recipes in JSON format.",
    "The first is the base recipe, the other two are clearly different alternatives.",
    "Each alternative must differ in at least one parameter.",
    ...
    "",
    `Brew method: ${params.brewMethodName}`,
    `Coffee weight (g): ${params.coffeeAmount}`,
    temperatureRule,
    comment,
  ].join("\n");
}
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a barista assistant. Respond only with JSON that matches the specification.",
        },
  ...
  try {
    ...
    if (!hasDistinctRecipes(normalizedRecipes)) {
      openRouterPayload = await callOpenRouter({
        apiKey: openRouterApiKey,
        model: openRouterModel,
        prompt: `${prompt}\n\nMake sure the alternatives differ in their parameters.`,
        referer: openRouterReferer || undefined,
        title: openRouterTitle || undefined,
        timeoutMs: openRouterTimeoutMs,
      });
