...
class EditRecipeViewModel @Inject constructor(
    ...
    private val _event: MutableSharedFlow<EditRecipeEvent> = MutableSharedFlow(extraBufferCapacity = 1)
    val event: SharedFlow<EditRecipeEvent> = _event.asSharedFlow()

    init {
        val mode = savedStateHandle.get<EditRecipeMode>("mode") ?: EditRecipeMode.MANUAL
    ...
    fun handleAction(action: EditRecipeAction) {
        when (action) {
            EditRecipeAction.NavigateBack -> navigateBack()
            EditRecipeAction.OpenBrewMethodSheet -> openBrewMethodSheet()
            EditRecipeAction.DismissBrewMethodSheet -> dismissBrewMethodSheet()
            is EditRecipeAction.SelectBrewMethod -> selectBrewMethod(action.brewMethodId)
            is EditRecipeAction.UpdateCoffeeAmount -> updateCoffeeAmount(action.value)
    ...
    private fun navigateBack() {
        sendEvent(EditRecipeEvent.NavigateBack)
    }
    ...
    private fun loadInitialData(recipeId: String?) {
        ...
        viewModelScope.launch {
            ...
            if (isUnsupportedStatus(recipe.status)) {
                updateState { it.copy(isLoading = false) }
                sendEvent(EditRecipeEvent.ShowMessage("This recipe can't be edited."))
                sendEvent(EditRecipeEvent.NavigateBack)
                return@launch
            }
            updateState { state ->
    ...
    private fun updateText(
        value: String,
        update: (EditRecipeUiState, String) -> EditRecipeUiState
    ) {
    ...
    private fun submit() {
        if (!_uiState.value.isSubmitEnabled) {
            sendEvent(EditRecipeEvent.ShowError("Fill in all required fields."))
            return
        }

        val validation = validateInputs()
        if (!validation.isValid) {
            sendEvent(EditRecipeEvent.ShowError("Fix the highlighted fields."))
            return
        }

        if (!hasChanges(_uiState.value)) {
            sendEvent(EditRecipeEvent.ShowMessage("No changes to save."))
            sendEvent(EditRecipeEvent.NavigateBack)
            return
        }
        ...
        viewModelScope.launch {
            when (_uiState.value.mode) {
                EditRecipeMode.MANUAL -> createManualRecipe(validation)
                EditRecipeMode.DRAFT -> updateDraftRecipe(validation)
                EditRecipeMode.SAVED -> updateSavedRecipe(validation)
            }
        }
    }
    ...
    private fun hasChanges(state: EditRecipeUiState): Boolean {
    ...
    private suspend fun createManualRecipe(validation: ValidationResult) {
        val brewMethodId = validation.brewMethodId ?: return showError("Missing brew method.")
        val input = CreateRecipe(
            generationRequestId = null,
            brewMethodId = brewMethodId,
            coffeeAmount = validation.coffeeAmount ?: return showError("Invalid coffee amount."),
            waterAmount = validation.waterAmount ?: return showError("Invalid water amount."),
            ratioCoffee = validation.ratioCoffee ?: return showError("Invalid ratio."),
            ratioWater = validation.ratioWater ?: return showError("Invalid ratio."),
            temperature = validation.temperature ?: return showError("Invalid temperature."),
            assistantTip = validation.assistantTip,
            status = RecipeStatus.Saved
        )
        handleRepositoryResult(createRecipeUseCase(input))
    }
    ...
    private suspend fun updateDraftRecipe(validation: ValidationResult) {
        ...
        val status = if (hasChanges(_uiState.value)) RecipeStatus.Edited else RecipeStatus.Saved
    ...
    private suspend fun updateSavedRecipe(validation: ValidationResult) {
        val recipeId = _uiState.value.recipeId ?: return showError("Missing recipe id.")
        val input = UpdateRecipe(
            brewMethodId = validation.brewMethodId,
            coffeeAmount = validation.coffeeAmount,
    ...
    private fun isUnsupportedStatus(status: RecipeStatus): Boolean {
        return status == RecipeStatus.Rejected || status == RecipeStatus.Deleted
    }

    private fun updateState(reducer: (EditRecipeUiState) -> EditRecipeUiState) {
        _uiState.value = reducer(_uiState.value)
    }

    private fun sendEvent(event: EditRecipeEvent) {
        viewModelScope.launch {
            _event.emit(event)
        }
    }
